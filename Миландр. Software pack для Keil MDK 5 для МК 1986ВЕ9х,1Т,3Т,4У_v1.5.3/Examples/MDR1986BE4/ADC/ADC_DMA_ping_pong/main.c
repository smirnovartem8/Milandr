...
uint32_t ADCConvertedValue1[SIZE_BUF_ADC];
uint32_t ADCConvertedValue2[SIZE_BUF_ADC];
...
{ 
 ...
 if( !(DMA_ControlTable[DMA_Channel_ADC].DMA_Control&0x7) || !(DMA_ControlTable[DMA_Channel_ADC + 32].DMA_Control&0x7) )
 {
    if((MDR_DMA->CHNL_PRI_ALT_SET & (1<<DMA_Channel_ADC)) == (0<<DMA_Channel_ADC))     // Сейчас выполняется основная структура
    {   
        DMA_CtrlDataInit(&DMA_AltCtrlStr_ADC, &DMA_ControlTable[DMA_Channel_ADC + 32]);
    }
    else if((MDR_DMA->CHNL_PRI_ALT_SET & (1<<DMA_Channel_ADC)) == (1<<DMA_Channel_ADC)) // Сейчас выполняется альтернативная структура
    {   
        DMA_CtrlDataInit(&DMA_PriCtrlStr_ADC, &DMA_ControlTable[DMA_Channel_ADC]);
    }
 }
}
...
void DMA_ADC1_Init(void)
{
  RST_CLK_PCLKcmd(RST_CLK_PCLK_DMA | RST_CLK_PCLK_PORTC, ENABLE);
  ...
  DMA_Init(DMA_Channel_ADC, &DMA_InitStr_ADC);
