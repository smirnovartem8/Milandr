...
/*
This demonstration code is intended to show the functionality
of microcontroller 1986VE1T in TERMINAL mode of the standart
GOST R 52070-2003.

This code write to microcontroller 1986VE1T rev. 6. In other
revision of MCU this code may performed with distinctions.

Author: Dyachkov Petr.
Date:		21.11.2016
*/
...
/*
Function for check new terminal address at parity
Parameters:	newAddress	- new terminal address in range 0 - 30
						parity			- parity bit (odd)
						
Returned value:	0 - parity OK
								1 - parity ERROR
*/
...
/*-----------------------------------------------------*/
/*---------- MIL_STD_15531 Interrupt Handler ----------*/
/*-----------------------------------------------------*/
	...
	if((_Status_ & MIL_STD_1553_STATUS_VALMESS) != 0)	//VALMESS interrupt occure
	{
		...
		{	
			if((_CommandWord1_ & SUBADDRESS) == (30 << 5))	//if 30 subaddress (subadderss for wraparound data)
			{
				...
				if(Control_Mode == 0) Control_Mode = 32;
...
int main (void)
{
	...
	{
		if((MDR_BKP->REG_0F & (1 << 22)) == 0)	MDR_BKP->REG_0F |= (1 << 22);	//power on HSI, if it disable
	...
	for(i = 0; i < 32; i++)
	{
		for(p = 0; p < 32; p++)		MDR_MIL_STD_15531->SubAddr[i].Data[p] = (((uint8_t)i) << 8)|((uint8_t)p);
	}
