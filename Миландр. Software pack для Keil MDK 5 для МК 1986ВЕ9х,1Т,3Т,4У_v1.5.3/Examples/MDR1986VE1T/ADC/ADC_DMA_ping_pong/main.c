...
{ 
 ...
 {
    if((MDR_DMA->CHNL_PRI_ALT_SET & (1<<DMA_Channel_SREQ_ADCDMA)) == (0<<DMA_Channel_SREQ_ADCDMA))     // Сейчас выполняется основная структура
    {   
        DMA_CtrlDataInit(&DMA_AltCtrlStr_ADC, &DMA_ControlTable[DMA_Channel_SREQ_ADCDMA + 32]);
    }
    else if((MDR_DMA->CHNL_PRI_ALT_SET & (1<<DMA_Channel_SREQ_ADCDMA)) == (1<<DMA_Channel_SREQ_ADCDMA)) // Сейчас выполняется альтернативная структура
    {   
