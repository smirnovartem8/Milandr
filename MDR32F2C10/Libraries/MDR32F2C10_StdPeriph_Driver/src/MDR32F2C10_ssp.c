...
/* Includes ------------------------------------------------------------------*/
#include "MDR32F2C10_ssp.h"

#define ASSERT_INFO_FILE_ID FILEID__MDR32F2C10_SSP_C
...
void SSP_CLK_en(MDR_SSP_TypeDef* SSPx, uint32_t SSP_CLKSRC, uint32_t SSP_CLK_DIV)
{
    uint32_t tmpreg;
    /* Check the parameters */
    assert_param(IS_SSP_ALL_PERIPH(SSPx));
    assert_param(IS_SSP_CLKSRC(SSP_CLKSRC));
    
    if( SSPx == MDR_SSP0 )
        tmpreg = MDR_CLK->SSP0_CLK;
    else if( SSPx == MDR_SSP1 ) 
        tmpreg = MDR_CLK->SSP1_CLK;
    ...
    if( SSPx == MDR_SSP0 )
        MDR_CLK->SSP0_CLK = tmpreg;
    else if( SSPx == MDR_SSP1 )
        MDR_CLK->SSP1_CLK = tmpreg;
}
