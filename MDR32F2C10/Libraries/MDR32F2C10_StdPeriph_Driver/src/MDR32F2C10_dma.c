...
/* Includes ------------------------------------------------------------------*/
#include "MDR32F2C10_dma.h"

#define ASSERT_INFO_FILE_ID FILEID__MDR32F2C10_DMA_C
...
{
    uint32_t req, mask;
    uint8_t muxn = DMA_Channel / 4;
    
    req = DMA_Req << ( 8 * ( DMA_Channel % 4 ) );
    mask = ~( 0xF << ( 8 * ( DMA_Channel % 4 ) ) );

    MDR_DMA->CHMUX[ muxn ] &= mask;
    MDR_DMA->CHMUX[ muxn ] |= req;
}
