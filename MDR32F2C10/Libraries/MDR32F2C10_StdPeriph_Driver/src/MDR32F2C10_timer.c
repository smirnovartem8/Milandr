...
#define ASSERT_INFO_FILE_ID FILEID__MDR32F2C10_TIMER_C
...
/**
  ...
  * @param  TIMER_BRG: specifies the MAX_CLK division factor.
  * @retval None
  */
void TIMER_BRGInit ( MDR_TIMER_TypeDef* TIMERx, uint32_t TIMER_BRG ) {
    ...
    switch( ( uint32_t )TIMERx )
    {
    case ( uint32_t )MDR_TIMER0:
        MDR_CLK->PER1_CLK |= 1 << 0;
        MDR_CLK->TIM0_CLK = 1<<16 | TIMER_BRG;
        break;
    case ( uint32_t )MDR_TIMER1:
        MDR_CLK->PER1_CLK |= 1 << 1;
        MDR_CLK->TIM1_CLK = 1<<16 | TIMER_BRG;        
        break;
    case ( uint32_t )MDR_TIMER2:
        MDR_CLK->PER1_CLK |= 1 << 2;
        MDR_CLK->TIM2_CLK = 1<<16 | TIMER_BRG;        
        break;
    case ( uint32_t )MDR_TIMER3:
        MDR_CLK->PER1_CLK |= 1 << 3;
        MDR_CLK->TIM3_CLK = 1<<16 | TIMER_BRG;        
        break;
    }
...
*
* END OF FILE MDR32F2C10_timer.c */
