/**
  ******************************************************************************
  * @file    MDR32F2C10_usb_device.c
  * @author  Milandr Application Team
  * @version V1.0.1
  * @date    11/11/2020
  * @brief   USB Library device source file.
  ******************************************************************************
  * <br><br>
...
#include "MDR32F2C10_usb.h"
#include "MDR32F2C10_usb_handlers.h"

#include <stdio.h>

#define ASSERT_INFO_FILE_ID    FILEID__MDR32F2C10_USB_DEVICE_C
...
USB_Result USB_EP_Init( USB_EP_TypeDef EPx, USB_EP_Error_Handler onError )
{
...
//    if (USB_IT & USB_SIS_SCTDONE) // Передача успешно завершена
//    {
//        ep->EP_WasScdone = SET;
//    }
...
//                        USB_SendData( EPx, Buffer, Length );
            }
//                }
...
//                    ep->Buffer.IO_Buffer.offset += count;
            ...
            USB_GetData( EPx, &ep->Buffer.IO_Buffer.pBuffer[ ep->Buffer.IO_Buffer.offset ] );
            ep->Buffer.IO_Buffer.offset += count;
...
//                {
            ...
            if ( count == 8 )
            {
//                        USB_EP0_GET((uint32_t*)ep->Buffer.pSetupPacket);
                USB_GetData( EPx, ( uint8_t* )ep->Buffer.pSetupPacket );
//                        for (i = 0; i < count; i++)
//                        {
...
static void USB_EP_sendInDataPortion( USB_EP_TypeDef EPx, USB_EPData_Bit_TypeDef DataBitChange )
{
    ...
    pBuffer = ep->Buffer.IO_Buffer.pBuffer;

    USB_SendData( EPx, &pBuffer[ ep->Buffer.IO_Buffer.offset ], total - ep->Buffer.IO_Buffer.offset,
                  DataBitChange );
}
...
USB_Result USB_DeviceInit( void )
{
    UsbInitCfg_t InitCfg;
...
USB_Result USB_DeviceSetupPacket( USB_EP_TypeDef EPx, const USB_SetupPacket_TypeDef* USB_SetupPacket )
{
    ...
    case USB_TYPE_STANDARD: {
        ...
        /* Recipient check */
        recipient =
            ( USB_RequestRecipient_TypeDef )( USB_SetupPacket->mRequestTypeData & USB_RECIPIENT_TYPE_Msk );
        ...
        case USB_GET_STATUS:
            if ( requestDirection != USB_DEVICE_TO_HOST ||
                 ( USB_DeviceContext.USB_DeviceState == USB_DEV_STATE_ADDRESS &&
                   ( recipient == USB_RECIPIENT_INTERFACE ||
                     recipient ==
                     USB_RECIPIENT_ENDPOINT && wIndex >= Num_USB_EndPoints ) ) )
            {
                result = USB_ERROR;
            }
            else
            {
                ...
                if ( result == USB_SUCCESS )
                {
                    ...
                    SetupPacketData[ 1 ] = 0;
                    result               = USB_EP_doDataIn( EPx, SetupPacketData, 2, NULL );
                }
            }
            break;
        /* CLEAR_FEATURE */
        ...
        case USB_SET_ADDRESS:
            ...
            MDR_USB0->INDEX       = 0;
            MDR_USB0->CSR0_TXCSR |= ( 1 << USB_CSR0_SRV_RX_PKT_RDY ) | ( 1 << USB_CSR0_DATA_END );
            /* Set special Status Stage handler that will actually change the address */
            USB_DeviceContext.Address = wValue;
            MDR_USB0->CSR0_TXCSR     |= ( 1 << USB_CSR0_FLUSH_FIFO );
            printf( "EP0 send ZLP addr\r\n" );
            return result;
//            statusAckHandler = USB_Device_setAddressWork;
            break;
        /* GET_DESCRIPTOR */
        case USB_GET_DESCRIPTOR:
            printf( "USB_GET_DESCRIPTOR\n" );
        ...
        case USB_GET_CONFIGURATION:
            ...
            result = USB_EP_doDataIn( EPx, SetupPacketData, 1, NULL );
            break;
        /* SET_CONFIGURATION */
        ...
        {
//            if( (USB_SetupPacket->mRequestTypeData & 0x80) != USB_DEVICE_TO_HOST )
//            {
//                MDR_USB0->CSR0_TXCSR |= (1 << 8);
//                printf("EP0 send ZLP\r\n");
//            }
...
USB_Result USB_DeviceDoStatusOutAck( USB_EP_TypeDef EPx, uint8_t* Buffer, uint32_t Length )
{
    ...
    MDR_USB0->CSR0_TXCSR |= ( 1 << USB_CSR0_FLUSH_FIFO );
    printf( "EP0 send ZLP\r\n" );
    return USB_SUCCESS;                                                                                        //USB_EP_doDataOut(EPx, 0, 0, 0);
}
...
{
    /* Set address */
    printf( "Set addr = %d", USB_DeviceContext.Address );
    USB_SetSA( USB_DeviceContext.Address );
    /* Adjust device state */
    USB_DeviceContext.USB_DeviceState =
        ( USB_DeviceContext.Address ? USB_DEV_STATE_ADDRESS : USB_DEV_STATE_DEFAULT );
...
void USB_IRQHandler( void )
{
    ...
    if ( 0 != USB_DeviceContext.Address )
    {
        MDR_USB0->FADDR = USB_DeviceContext.Address;
        printf( "Set addr = %d \r\n", MDR_USB0->FADDR );
        USB_DeviceContext.Address = 0;
    }

    /* Get masked interrupt flags */
    UsbIntr = MDR_USB0->INTRUSB;

    if ( UsbIntr & ( 1 << USB_INTR_RST_BIT ) )
    {
        printf( "reset\r\n" );
        USB_DEVICE_HANDLE_RESET;
    }

    uint16_t TxIntr = MDR_USB0->INTRTX;
    uint16_t RxIntr = MDR_USB0->INTRRX;
    printf( "UsbIntr 0x%x, TxIntr 0x%x, RxIntr 0x%x\r\n", UsbIntr, TxIntr, RxIntr );
...
/* Invoke End Point dispatchers */
    ...
    if ( TxIntr & 0x1 )
    {
        printf( "IN0\n\r" );
        MDR_USB0->INDEX = 0;
        csr             = MDR_USB0->CSR0_TXCSR;
        printf( "csr = %d, size_rx = %d\r\n", csr, USB_GetRxCnt( 0 ) );

        if ( csr & ( 1 << USB_CSR0_SETUP_END ) /*||
                                                  csr & ( 1 << USB_CSR0_SENT_STALL )*/)
        {
//            MDR_USB0->CSR0_TXCSR &= ~(1 << USB_CSR0_SENT_STALL);
//            MDR_USB0->CSR0_TXCSR |= (1 << USB_CSR0_FLUSH_FIFO);
//            MDR_USB0->CSR0_TXCSR |= (1 << USB_CSR0_SEND_STALL);
//            USB_DeviceContext.Address = 0;
            MDR_USB0->CSR0_TXCSR |= ( 1 << USB_CSR0_SRV_SETUP_END );
            printf( "Setup end\r\n" );
        }

        if ( csr & ( 1 << USB_CSR0_RX_PKT_RDY ) )
            USB_EP_dispatchEvent( USB_EP0, UsbIntr );
    }
    if ( TxIntr & 0x2 )
        USB_EP_dispatchEvent( USB_EP1, UsbIntr );
    if ( TxIntr & 0x4 )
        USB_EP_dispatchEvent( USB_EP2, UsbIntr );
    if ( TxIntr & 0x8 )
        USB_EP_dispatchEvent( USB_EP3, UsbIntr );
...
/* END OF FILE MDR32F2C10_usb_device.c */
