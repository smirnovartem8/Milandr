/**
  ******************************************************************************
  * @file    MDR32F2C10_usb_CDC.c
  * @author  Milandr Application Team
  * @version V1.0.1
  * @date    11/11/2020
  * @brief   USB Communication Device Class source file.
  ******************************************************************************
  * <br><br>
...
#define ASSERT_INFO_FILE_ID    FILEID__MDR32F2C10_USB_CDC_C
...
USB_Result USB_CDC_SendData( uint8_t* Buffer, uint32_t Length )
{
    ...
    if ( result == USB_SUCCESS )
    {
        ...
        result                            =
            USB_EP_doDataIn( USB_CDC_EP_SEND, Buffer, Length, USB_CDC_OnDataSent );
    }
    return result;
}
...
USB_Result USB_CDC_ReportState( uint16_t LineState )
{
    ...
    return USB_EP_doDataIn( USB_CDC_EP_STATE, ( uint8_t* )&CDC_StateReportPacket,
                            sizeof( CDC_StateReportPacket ), NULL );
}
#endif                                                                                                         /* USB_CDC_STATE_REPORTING_SUPPORTED */
...
USB_Result USB_CDC_Reset( void )
{
    ...
    if ( result == USB_SUCCESS )
    {
        /* EP1, EP2 and EP3 initialization */
        epCfg.DataPtr         = NULL;
        epCfg.EP_Num          = USB_CDC_EP_SEND;
        epCfg.EP_Type         = Bulk;
        epCfg.Direction       = Tx;
        epCfg.AutoHandling    = 0;
        epCfg.PktMaxSize      = MAX_PACKET_SIZE;
        epCfg.DoubleBuffering = 0;
        USB_InitEP( &epCfg );
        USB_EP_Init( USB_CDC_EP_SEND, USB_CDC_HANDLE_SEND_ERROR );

        epCfg.EP_Num    = USB_CDC_EP_STATE;
        epCfg.EP_Type   = Irq;
        epCfg.Direction = Tx;
        USB_InitEP( &epCfg );
        USB_EP_Init( USB_CDC_EP_STATE, USB_CDC_HANDLE_LINE_STATE_REPORT_SEND_ERROR );

        epCfg.EP_Num    = USB_CDC_EP_RECEIVE;
        epCfg.EP_Type   = Bulk;
        epCfg.Direction = Rx;
        USB_InitEP( &epCfg );
        USB_EP_Init( USB_CDC_EP_RECEIVE, 0 );
...
//        USB_EP0_SEND( (uint8_t * )pDescr, length);
    }
    return result;
}
...
USB_Result USB_CDC_ClassRequest( void )
{
    ...
    case USB_CDC_GET_COMM_FEATURE:
        if ( wLength == 2 )
        {
            ...
            if ( result == USB_SUCCESS )
            {
                result = USB_EP_doDataIn( USB_EP0, ( uint8_t* )&CDC_ClassPacketData, wLength,
    ...
    case USB_CDC_SET_LINE_CODING:
        printf( "USB_CDC_SET_LINE_CODING\r\n" );
        if ( wLength == 7 )
        {
            result = USB_EP_doDataOut( USB_EP0, ( uint8_t* )&CDC_ClassPacketData, wLength, USB_CDC_DoDataOut );
        }
    ...
    if ( result == USB_SUCCESS && wLength == 0 )
    {
        result = ( USB_CurrentSetupPacket.mRequestTypeData & 0x80 ) == USB_DEVICE_TO_HOST ?
                 USB_EP_doDataOut( USB_EP0, 0, 0, NULL ) :
                 USB_EP_doDataIn( USB_EP0, 0, 0, NULL );
    }
...
static USB_Result USB_CDC_DoDataOut( USB_EP_TypeDef EPx, uint8_t* Buffer, uint32_t Length )
{
    ...
    return result == USB_SUCCESS ? USB_EP_doDataIn( EPx, 0, 0, NULL ) : result;
}
...
/* END OF FILE MDR32F2C10_usb_CDC.c */
